...
- **NFR-005**: Performance targets MUST be defined for critical operations (see plan.md).
...
- **NFR-008**: New operations MUST emit structured JSON logs indexable in Kibana.
- **NFR-009**: CI pipeline MUST run lint, security audit, tests and coverage gate on every PR.
- **NFR-010**: Entidades de negócio novas DEVEM usar PKs/FKs `BIGINT UNSIGNED AUTO_INCREMENT`
  (Princípio IX); contratos OpenAPI com `integer`/`int64`; exceções (tokens opacos)
  documentadas no `data-model.md`.
...
- [Assumption about scope boundaries, e.g., "Frontend/mobile live in separate repositories"]
